...
class PatternMaker:
    ...
    def save(self):
        absolute_path = os.path.abspath(self.output)
        c = canvas(self.g, width="%d%s" % (self.width, self.units), height="%d%s" % (self.height, self.units),
                   viewBox="0 0 %d %d" % (self.width, self.height))

        c.save(absolute_path)
        if os.path.exists(absolute_path):
            try:
                os.startfile(absolute_path)
            except Exception as e:
                print(f"️Impossible d’ouvrir le fichier : {e}")
        else:
            print("Erreur : le fichier SVG n’a pas été trouvé.")
...
def main():
    ...
    print("Je sauvegarde le chessboard")
    pm.save()
